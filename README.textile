...
5. It builds artifacts using the configured Build strategy (see the Build section below)
...
This can be used to explicitly specify the build strategy that will be used.  The currently supported values for this are:
...
The build strategy is determined by the following:

* if @:build-via@ is specified in the configuration, is value is used
* otherwise @:lein-jar-pom@ is used

Additional build strategies can be added by other lein plugins that depend
on lein-release and define methods @build@.
